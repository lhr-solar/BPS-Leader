...
static uint8_t temp_can_unpack(uint8_t *raw_temp_can_data, bps_temperature_aggregate_arr_t *temp_can_data)
{
    ...
    if (raw_temp_can_data == NULL) return 0;

    uint8_t tap_index = raw_temp_can_data[0] & temp_id_mask;

    if(tap_index >= NUM_TEMPERATURE_SENSORS) return 0;
    ...
    temp_can_data[tap_index].BPS_Temperature_Tap_Age = Calculate_TimeDifference(pdMS_TO_TICKS(xTaskGetTickCount()) , pdMS_TO_TICKS(temp_can_data[tap_index].BPS_Temperature_Tap_Age));
