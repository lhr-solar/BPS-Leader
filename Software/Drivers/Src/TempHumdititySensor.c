...
static void sensorPoll(uint8_t* rx_bytes) {
  ...
  // transmit data
   if (HAL_I2C_Master_Transmit_IT(&hi2c4, tmpHmdAdresss << 1, &pollCMD, tx_size) != HAL_OK) {
    errorHandler();
   }
  ...
  // recieve data
  if (HAL_I2C_Master_Receive_IT(&hi2c4, tmpHmdAdresss << 1, rx_bytes, rx_size) != HAL_OK) {
    errorHandler();
  }
...
void readTmpHmd(double* tmpHmdBuffer) {
  ...
  tmpHmdBuffer[0] = -45.0 + 175.0 * t_ticks/65535.0;
  tmpHmdBuffer[1] = -6.0 + 125.0 * rh_ticks/65535.0;
