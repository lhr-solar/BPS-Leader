...
void init_contactors() {
    ...
    // init channel A contactor sense pins 
    ...
    // init channel B contactor sense pins
    GPIO_InitStruct.Pin = ARRAY_PRE_SENSE_PIN_NUM;
    GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
    HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);

    // init channel B contactor control pins
    ...
    // init channel C contactor  control pins
...
void vContactorCallback( TimerHandle_t senseTimer ) {
    ...
    // take mutex for a short time just to confirm check is accurate
    if (xSemaphoreTake(contactorsMutex, CALLBACK_BLOCKING_TIME) == pdFALSE) {
        // Someone is using the contactor sephmore for a lot longer than they should. call fault handler.
    }
    ...
    if (xSemaphoreGive(contactorsMutex) == pdFALSE) {
        // this really shouldn't happen. call fault handler.
    }
 
}

ErrorStatus toggle_contactor(contactor_enum_t contactor_num, bool state, bool blocking, bool emergency) {
